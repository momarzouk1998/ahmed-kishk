...
export function normalizeMasterStage(statusStr: string): GlobalMasterStage {
  ...
  if (s === 'المعاينات' || s === 'في المعاينات' || s.includes('معاينة') || s === 'مُجدول' || s === 'تم رفع المقاسات') return 'المعاينات';
  if (s === 'انتظار تسعير' || s === 'في التسعير' || s.includes('تسعير') || s === 'قيد التسعير' || s === 'بانتظار التسعير' || s === 'تم إرسال المقايسة' || s === 'تم ارسال المقايسة') return 'انتظار تسعير';
  if (s === 'في المقص' || s === 'بانتظار القص' || s === 'قص القماش' || s === 'تم التحويل للقص' || s === 'معتمد ومسدد العربون' || s === 'معتمد و مسدد العربون') return 'في المقص';
  if (s === 'في الورشة' || s === 'جاري الخياطة' || s === 'جاري الكي' || s === 'تمت الخياطة' || s === 'تم القص وجاهز للخياطة' || s === 'تم التحويل للورشة' || s === 'تم التحويل الى الورشه' || s === 'تم القص') return 'في الورشة';
  if (s === 'تجهيز الاكسسوارات' || s === 'تجهيز الاكسسوار' || s === 'جاري التجهيز' || s === 'تم التجهيز') return 'تجهيز الاكسسوارات';
  if (s === 'جاهز للاستلام' || s === 'في التسليمات' || s === 'تسليمات المعرض والشحن' || s === 'جاهز للتسليم' || s === 'جاهز للستليم') return 'جاهز للاستلام';
  if (s === 'جاهز للتركيب' || s === 'في التركيبات' || s === 'التركيبات') return 'جاهز للتركيب';
  if (s === 'مكتمل' || s === 'تم التسليم للعميل بنجاح' || s === 'تم التركيب بنجاح ومغلق' || s === 'مكتمل ومسلم' || s === 'تم التركيب بنجاح' || s === 'تم التسليم بنجاح' || s === 'تم التسليم') return 'مكتمل';
  return 'المعاينات';
}
