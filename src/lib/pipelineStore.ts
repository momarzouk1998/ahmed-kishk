...
export async function updatePipelineOrderStatus(
  ...
  if (!found && cleanId) {
    ...
    const newEntry: PipelineMasterOrder = {
      ...
      inspectionDate: quotMatch?.inspectionDate || quotMatch?.date || '',
      installationDate: quotMatch?.installationDate || (quotMatch?.fulfillmentType === 'INSTALLATION' ? quotMatch?.deliveryDate : '') || '',
      scheduledDate: quotMatch?.installationDate || (quotMatch?.fulfillmentType === 'INSTALLATION' ? quotMatch?.deliveryDate : '') || '',
      fulfillmentType: quotMatch?.fulfillmentType || 'INSTALLATION',
